...
class MultimodalGRPOTrainer(Trainer):
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                if vllm_device == "auto":
                    ...
                    # vllm_device = f"cuda:{self.accelerator.local_process_index}"
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.use_vllm:
            ...
            all_prompts_text = gather_object(temp_inputs)
            all_image_array_image = gather_object(inputs["array_image"])
            
            
            if self.accelerator.is_main_process:
                print(len(all_prompts_text))
                print(len(all_image_array_image))
                all_completions = []
